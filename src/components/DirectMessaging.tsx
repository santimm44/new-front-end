...
const DirectMessaging = () => {
  ...
  const enterChatRoom = async (grabbedId: number, grabbedUsername: string) => {
    ...
    const getData = async () => {
      try {
        if (userId) {
          const res = await fetch(`http://localhost:5177/Message/Conversation/${userInformation?.id}/${grabbedId}`)
  ...
  const saveMessage = async (grabbedId: number, timeStamp: string) => {
    if (newMessage.trim()) {
      try {
        ...
        await fetch(`http://localhost:5177/Message/SaveMessage/${newMessage}/${userInformation?.id}/${timeStamp}/${grabbedId}`, {
          method: 'POST',
        })
